...
export function PaidDutyListScreen({ duties, onChange }: { duties: PaidDuty[]; onChange: (duties: PaidDuty[]) => void }) {
  ...
  return (
    <View style={styles.wrap}>
      ...
      {filtered.length === 0 ? <EmptyState actionLabel="Add Paid Duty" icon="briefcase-outline" onAction={() => { setSelectedId(null); setMode("form"); }} title={`No ${filter} duties`} message="Paid Duty details are stored locally on this device." /> : (
